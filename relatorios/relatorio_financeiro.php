...
/* =========================
   AJAX - POPUP VARIADO
========================= */
if (isset($_GET['ajax']) && $_GET['ajax'] === 'variado') {
...
if (!empty($_GET['mes'])) {
        $mesInicio = $_GET['mes'] . '-01';
        $mesFim = date('Y-m-d', strtotime($mesInicio . ' +1 month'));

        $where[] = 'p.DataPedido >= :mesInicio AND p.DataPedido < :mesFim';
        $params[':mesInicio'] = $mesInicio;
        $params[':mesFim'] = $mesFim;
    }elseif (!empty($_GET['data_inicio']) && !empty($_GET['data_fim'])) {
        $where[] = 'p.DataPedido BETWEEN :di AND :df';
        $params[':di'] = $_GET['data_inicio'];
        $params[':df'] = $_GET['data_fim'];
    }

    $where[] = 'p.ValorVariado > 0';
    ...
    $sql = "
        SELECT 
            p.IdPedido,
            DATE_FORMAT(p.DataPedido, '%d/%m/%Y') AS DataPedido,
...
if ($mes !== '') {
    ...
    $proximoMes = date('Y-m-d', strtotime($mesInicio . ' +1 month'));

    $where[] = 'p.DataPedido >= :mesInicio AND p.DataPedido < :mesFim';
    $params[':mesInicio'] = $mesInicio;
    $params[':mesFim'] = $proximoMes;
} elseif ($dataInicio !== '' && $dataFim !== '') {
    $where[] = 'p.DataPedido BETWEEN :di AND :df';
    $params[':di'] = $dataInicio;
    $params[':df'] = $dataFim;
...
<tr>
    ...
    <td>
    <?php if ($p['Produto'] === '🛒 - VARIADO'): ?>
        <a href="#" class="openVariado"><?= $p['Qtd'] ?></a>
    <?php else: ?>
        <?= $p['Qtd'] ?>
    <?php endif; ?>
    </td>
...
<div id="modalVariado" style="display:none;
    position:fixed; inset:0; background:rgba(0,0,0,.8);
    align-items:center; justify-content:center; z-index:9999;">
    
    <div style="background:#181818; padding:20px; width:90%; max-width:800px;
        border-radius:10px; max-height:80vh; overflow:auto;">
        
        <h3>🛒 Detalhes de Valores Variados</h3>
        <table style="width:100%; margin-top:10px;">
            ...
            <tbody id="modalVariadoBody"></tbody>
        </table>

        <button onclick="fecharVariado()" style="margin-top:15px;">Fechar</button>
    </div>
</div>
...
document.addEventListener('click', function (e) {
    ...
    if (!e.target.classList.contains('openVariado')) return;

    e.preventDefault();

    const modal = document.getElementById('modalVariado');
    const tbody = document.getElementById('modalVariadoBody');

    tbody.innerHTML = '<tr><td colspan="5">Carregando...</td></tr>';

    // Monta a URL do fetch incluindo todos os filtros do formulário manualmente
    const form = document.getElementById('filtrosForm');
    const formData = new FormData(form);

    // Converte FormData para query string
    const params = new URLSearchParams(formData);

    // Adiciona ajax=variado para o endpoint reconhecer a requisição ajax
    params.set('ajax', 'variado');

    // Faz o fetch com os filtros aplicados
            ...
            dados.forEach(l => {
                tbody.innerHTML += `
                    <tr>
                        <td>${l.IdPedido}</td>
                        <td>${l.DataPedido}</td>
                        <td>${l.NomeCompletoCliente}</td>
                        <td>R$ ${parseFloat(l.ValorVariado).toFixed(2)}</td>
                        <td>${l.ObservacaoVariado ?? ''}</td>
                    </tr>
                `;
            });
        })
...
function fecharVariado() {
    document.getElementById('modalVariado').style.display = 'none';
}
